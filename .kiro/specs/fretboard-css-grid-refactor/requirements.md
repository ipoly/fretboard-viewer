...
- **String_Row**: 网格中代表弦的行
- **Open_String_Column**: 网格的第一列，用于显示空弦标记
- **Sticky_Position**: CSS 定位属性，使元素在滚动时保持固定位置
- **Grid_Layer**: 通过 z-index 控制的网格层级，允许元素堆叠
- **Fretboard_Grid**: 重构后的指板网格组件
- **Position_Calculation**: 当前基于像素计算的定位方式
- **Grid_Positioning**: 基于网格行列的定位方式
...
2. THE Fretboard_Grid SHALL 定义网格列数等于品格数量加一（包含空弦列）
3. THE Fretboard_Grid SHALL 定义网格行数等于弦的数量（6行）
4. THE Fretboard_Grid SHALL 支持网格元素的层级堆叠通过 z-index 控制
5. THE Fretboard_Grid SHALL 保持与现有实现相同的视觉外观和尺寸比例

### Requirement 2: 空弦标记定位

**User Story:** 作为用户，我希望空弦标记始终可见且正确对齐，即使在水平滚动时也能保持固定位置。

#### Acceptance Criteria

1. THE Open_String_Column SHALL 位于网格的第一列（column 1）
2. THE Open_String_Column SHALL 使用 sticky positioning 和 left: 0 保持固定位置
3. WHEN 用户水平滚动指板时，THE Open_String_Column SHALL 保持在视口左侧可见
4. THE Open_String_Column SHALL 与对应弦行完美对齐
5. THE Open_String_Column SHALL 显示空弦音符（如果在选定调式中）
6. THE Open_String_Column SHALL 具有与指板底色相同的背景，确保在滚动时能够遮挡后面的内容
...
1. THE 弦线 SHALL 从网格的第二列开始绘制（column 2 开始）
2. THE 品丝 SHALL 从网格的第二列开始绘制（column 2 开始）
...
### Requirement 4: 音符标记网格定位

**User Story:** 作为开发者，我希望音符标记能够通过弦号和品格号简单定位，提高代码可读性和维护性。

#### Acceptance Criteria

1. THE 音符标记 SHALL 使用网格坐标 (string_number, fret_number) 进行定位
2. WHEN 定位音符标记时，THE Fretboard_Grid SHALL 使用 grid-row 对应弦号
3. WHEN 定位音符标记时，THE Fretboard_Grid SHALL 使用 grid-column 对应品格号加一（因为第一列是空弦列）
4. THE 音符标记 SHALL 自动居中对齐在对应的网格单元格中
5. THE 音符标记 SHALL 不需要任何像素位置计算
...
3. THE 音符标记 SHALL 位于较高层级（z-index: 3）
4. THE 空弦标记 SHALL 位于最顶层（z-index: 4），高于其他音符标记
5. THE 空弦标记 SHALL 具有与指板底色相同的背景层，确保在粘性定位时能够遮挡后面的元素
6. THE 层级系统 SHALL 确保所有元素正确显示且不被遮挡
...
3. THE 空弦列宽度 SHALL 在所有设备上保持适当的比例
...
3. THE Fretboard_Grid SHALL 使用声明式的 CSS Grid 属性替代命令式的位置计算
4. THE Fretboard_Grid SHALL 减少组件代码行数至少 30%
5. THE Fretboard_Grid SHALL 提高代码可读性和维护性
